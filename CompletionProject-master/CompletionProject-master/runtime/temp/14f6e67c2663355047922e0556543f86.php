<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:65:"C:\wamp64\www\tp1\public/../ems/mangeuser\view\shop\userlist.html";i:1539590371;s:54:"C:\wamp64\www\tp1\ems\mangeuser\view\public\_head.html";i:1539589474;}*/ ?>
<!DOCTYPE html>
<html lang="en">
...
<div class="rightArtical rightContent">
    <div class="rightTitel">
        <nav class="breadcrumb">
            <span><i class="fa fa-user-circle"></i> 管理员管理&nbsp;<b>/</b>&nbsp;管理员列表</span>
        </nav>
    </div>
            ...
            <table class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td id="name"><?php echo $vo['username']; ?></td>
                        <td><?php echo $vo['nickname']; ?></td>
                        <td><?php if($vo['statue'] == 1): ?>
                        ...
                        <td><?php if($vo['is_super'] == 1): ?>
                            <span class="text-success">超级管理员</span>
                            <?php else: ?>
                            <span class="text-secondary">普通管理员</span>
                            <?php endif; ?>
                        </td>
                        <td><?php echo $vo['email']; ?></td>
                        <td>
                            <?php if(session('is_super') == 1): if($vo['is_super'] != 1): if($vo['statue'] == 1): ?>
                            <button id="<?php echo $vo['statue']; ?>" dataid="<?php echo $vo['id']; ?>" class="btn btn-secondary is_super">禁用</button>
                            <?php else: ?>
                            <button id="<?php echo $vo['statue']; ?>" dataid="<?php echo $vo['id']; ?>" class="btn btn-success is_super">启用</button>
                            <?php endif; ?>

                            <button id="<?php echo $vo['id']; ?>" class="btn btn-danger deleteUser">删除</button>
                            <?php endif; else: ?>
                            <span class="text-danger">你不是超级管理员~无操作！</span>
                            <?php endif; ?>
                        </td>
                    </tr>
...
<script>
    $(function () {
        ...
        $('.deleteUser').click(function (e) {
            ...
            var data = { "id": id }
            layer.confirm('您确定要删除吗?', { icon: 3, title: '请谨慎删除管理员！' }, function (index) {
                $.ajax({
                    type: "post",
                    url: "<?php echo url('mangeuser/shop/userdelete'); ?>",
                    data: data,
                    dataType: "json",
                    success: function (data) {
                        if (data.code == 1) {
        ...
        //会员状态
        $('.is_super').click(function (e) {
            e.preventDefault();
            var statue = $(this).attr("id");
            var id = $(this).attr("dataid");
            var data = { "statue": statue, "id": id };
            var msg;
            if (statue == 1) {
                msg = '已禁用！';
            } else {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo url('mangeuser/shop/userlist'); ?>",
                data: data,
                dataType: "json",
                success: function (data) {
                    if (data.code == 1) {
